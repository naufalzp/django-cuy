...
with open('./csv_data/user-data.csv', encoding='utf-8-sig') as csvfile:
        reader = csv.DictReader(csvfile)
        first_row = next(reader, None)
        print("First row from user-data.csv:", first_row)
...
with open('./csv_data/course-data.csv', encoding='utf-8-sig') as csvfile:
    reader = csv.DictReader(csvfile)
    for num,row in enumerate(reader):           
...
with open('csv_data/member-data.csv', encoding='utf-8-sig') as csvfile:
    reader = csv.DictReader(csvfile)
    for num, row in enumerate(reader):
